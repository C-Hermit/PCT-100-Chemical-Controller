...
void oled_ui_refresh(bool main_power, bool is_auto, unsigned int light, unsigned int light_th, float temp, float temp_th, bool led_on, bool fan_on) {
    ...
    snprintf(buf, sizeof(buf), "模式: %s", is_auto ? "自动" : "手动");
    u8g2.drawUTF8(0, 0, buf);
    snprintf(buf, sizeof(buf), "总闸: %s", main_power ? "ON" : "OFF");
    u8g2.drawUTF8(70, 0, buf); 

    // 2. 第二行：LIGHT 实时值 / 阈值 (y = 16)
    snprintf(buf, sizeof(buf), "光照: %u/%u", light, light_th);
    u8g2.drawUTF8(0, 16, buf);

    // 3. 第三行：TEMP 实时值 / 阈值 (y = 32)
    snprintf(buf, sizeof(buf), "温度: %.1f/%.1f", temp, temp_th);
    u8g2.drawUTF8(0, 32, buf);

    // 4. 第四行：LED 与 FAN 状态 (y = 48)
    snprintf(buf, sizeof(buf), "灯光: %s", led_on ? "ON" : "OFF");
    u8g2.drawUTF8(0, 48, buf);
    snprintf(buf, sizeof(buf), "风扇: %s", fan_on ? "ON" : "OFF");
